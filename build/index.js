...
function _godot_webxr_commit_for_eye(p_eye, p_destination_fbo) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(49, 1, p_eye, p_destination_fbo);
...
function _godot_webxr_get_controller_transform(p_controller) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(54, 1, p_controller);
 if (!GodotWebXR.session || !GodotWebXR.frame) {
  return 0;
 }
 const controller = GodotWebXR.controllers[p_controller];
...
function _godot_webxr_get_projection_for_eye(p_eye) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(55, 1, p_eye);
 if (!GodotWebXR.session || !GodotWebXR.pose) {
  return 0;
 }
 const view_index = p_eye === 2 ? 1 : 0;
...
function _godot_webxr_get_transform_for_eye(p_eye) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(57, 1, p_eye);
 if (!GodotWebXR.session || !GodotWebXR.pose) {
  return 0;
 }
...
function _godot_webxr_initialize(p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_controller_changed, p_on_input_event, p_on_simple_event) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(60, 1, p_session_mode, p_required_features, p_optional_features, p_requested_reference_spaces, p_on_session_started, p_on_session_ended, p_on_session_failed, p_on_controller_changed, p_on_input_event, p_on_simple_event);
...
function _godot_webxr_is_controller_connected(p_controller) {
 if (ENVIRONMENT_IS_PTHREAD) return _emscripten_proxy_to_main_thread_js(61, 1, p_controller);
 if (!GodotWebXR.session || !GodotWebXR.frame) {
  return false;
 }
...
var dynCall_jijjjiiiiiiii = Module["dynCall_jijjjiiiiiiii"] = createExportWrapper("dynCall_jijjjiiiiiiii");
...
var dynCall_vijijiiif = Module["dynCall_vijijiiif"] = createExportWrapper("dynCall_vijijiiif");
